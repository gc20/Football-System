...
void initializeGame() {
    ...
    currBall.speed = BALL_INITIAL_SPEED;
    currBall.direction = BALL_INITIAL_DIRECTION;
...
void strategy() {
	...
	double min_dist1 = playerPlayerDistance(&(teamB).player[0], &(teamA).player[0]); // distance of the closest player
	u8 min_player1 = 0;
	double min_dist2 = playerPlayerDistance(&(teamB).player[1], &(teamA).player[0]); // distance of the second closest player
	u8 min_player2 = 1;
	u8 i = 2;			 
...
player* playerClosestToBall (assignment playerAssign[]) {
    ...
    double min_dist = playerBallDistance(&(teamA.player[1]));
    u8 closest_player = 1;
	double temp_distance;
	u8 i;
...
double playerBallDistance (player* myPlayer) {
	
	double dx = (currBall).position.x - (*myPlayer).position.x;
	double dy = (currBall).position.y - (*myPlayer).position.y;
	return ( (dx*dx + dy*dy));
}

// calculate distance between centers of two players
double playerPlayerDistance (player* opponentPlayer, player* myPlayer) {
	
	double dx = (*opponentPlayer).position.x - (*myPlayer).position.x;
	double dy = (*opponentPlayer).position.y - (*myPlayer).position.y;
	return ((dx*dx + dy*dy));
}
